...
function synthesizeTranslation(translations) {
    ...
    string  += translations[i]["translation"]["translatedText"];
    ...
    return fetch(serverUrl + "/synthesize_speech", {
        ...
        //body: JSON.stringify({text: translations[0]["translation"]["translatedText"]})
    }).then(response => {
        if (response.ok) {
            return response.json();
        } else {
...
function uploadAndTranslate() {
    uploadImage()
        ...
        .then(translations => synthesizeTranslation(translations))
